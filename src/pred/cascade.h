...
class BlendLS {
  public:
    ...
    double GetWeightRaw(std::int32_t index) {
    ...
    //blended contribution of index component over all experts
    double GetWeight(std::int32_t index) {
...
make_mix(std::int32_t n,double mu_mix,double mu_mix_beta)
{
...
class Cascade {
  public:
    Cascade(Range r,const std::vector<std::int32_t> &vn,const std::vector<double>&vmu,
               const std::vector<double>&vmudecay,const std::vector<double> &vpowdecay,
               double mu_mix,double mu_mix_beta,std::int32_t lm_n,double lm_alpha,double proj_alpha)
    :r(r),n(vn.size()),p(n+1),bp(n+1),
