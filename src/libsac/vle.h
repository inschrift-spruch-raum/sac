  ...
  StaticLaplaceModel(std::int32_t maxbpn):
    ...
    for(std::int32_t sum = 0; sum < (1 << maxbpn); sum++) {
      for(std::int32_t bpn = 0; bpn < 32; bpn++) {
        ...
        std::int32_t pi =
          std::clamp((std::int32_t)std::round(pd * PSCALE), 1, PSCALEm);
        pr[sum][bpn] = pi;
      }
    }
  }
...
public:
  BitplaneCoder(std::int32_t maxbpn, std::size_t numsamples);
...
private:
  ...
  std::vector<LinearCounterLimit> csig0, csig1, csig2, csig3, cref0, cref1,
    cref2, cref3;
...
class Golomb {
public:
  Golomb(RangeCoderSH& rc): msum(0.98, 1 << 15), rc(rc) { lastl = 0; }
  ...
  void Encode(std::int32_t val) {
    ...
    // for (std::int32_t i=0;i<q;i++) rc.EncodeBitOne(PSCALEh,1); // encode
    // exponent unary rc.EncodeBitOne(PSCALEh,0);
    ...
    if(m > 1) {
      ...
      std::int32_t t = (1 << b) - m;
      if(r < t) {
        for(std::int32_t i = b - 2; i >= 0; i--)
          rc.EncodeBitOne(PSCALEh, ((r >> i) & 1));
      } else {
        for(std::int32_t i = b - 1; i >= 0; i--)
          rc.EncodeBitOne(PSCALEh, (((r + t) >> i) & 1));
      }
    }
...
class GolombRC {
public:
  GolombRC(RangeCoder& rc): msum(0.8, 1 << 15), rc(rc) {}
  ...
  void Encode(std::int32_t val) {
    ...
    for(std::int32_t i = 0; i < q; i++)
      rc.EncodeBitOne(PSCALEh, 1); // encode exponent unary
